...
contract NFTMarketplaceWithERC20 is ReentrancyGuard {
    ...
    function buyGroup(uint16 group, address nftContract, uint256 maxTotal) external returns (uint256[] memory purchased) {
        uint256[] memory tokenIds = groupTokenIds[group][nftContract];
        purchased = new uint256[](tokenIds.length);
        uint256 count = 0;
        uint256 totalSpent = 0;
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            if (listings[nftContract][tokenId] == address(0)) continue;
            uint256 price = getGroupPrice(group);
            if (totalSpent + price > maxTotal) {
                emit PurchaseFailed(nftContract, tokenId, "Budget exceeded");
                continue;
            }
            if (!paymentToken.transferFrom(msg.sender, listings[nftContract][tokenId], price)) {
                emit PurchaseFailed(nftContract, tokenId, "Payment failed");
                continue;
            }
            address seller = listings[nftContract][tokenId];
    ...
    function getAllGroupTokenIds(uint16 group, address nftContract) external view returns (uint256[] memory) {
        return groupTokenIds[group][nftContract];
    }
}
