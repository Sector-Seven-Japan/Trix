...
const marketplaceAbi = [
    "function listNFT(address nftContract, uint256 tokenId, uint256 price, string memory group) external",
    ...
    "function getListing(address nftContract, uint256 tokenId) external view returns (tuple(uint256 price, address seller, string group))",
    "function groupBids(string) external view returns (tuple(uint256 price, address buyer))",
    "function listedNFTs(uint256) external view returns (tuple(address nftContract, uint256 tokenId))",
    "function listings(address, uint256) external view returns (tuple(uint256 price, address seller, string group))",
...
app.get("/marketplace/groupbid/:group", async (req, res) => {
    try {
        ...
        console.error("Get group bid error:", error);
        res.status(500).json({ error: error.message });
    }
});
...
app.get("/marketplace/listed-nfts", async (req, res) => {
    try {
        ...
        while (true) {
            try {
                const nft = await marketplaceContract.listedNFTs(index);
                nfts.push({
